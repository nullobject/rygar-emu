...
void mem_init(mem_t* mem);
/* map a range of RAM */
void mem_map_ram(mem_t* mem, int layer, uint32_t addr, uint32_t size, uint8_t* ptr);
/* map a range of ROM */
void mem_map_rom(mem_t* mem, int layer, uint32_t addr, uint32_t size, const uint8_t* ptr);
/* map a range of memory to different read/write pointers (e.g. for RAM behind ROM) */
void mem_map_rw(mem_t* mem, int layer, uint32_t addr, uint32_t size, const uint8_t* read_ptr, uint8_t* write_ptr);
...
uint8_t mem_layer_rd(mem_t* mem, int layer, uint32_t addr);
/* write a byte to a specific layer (slow!) */
void mem_layer_wr(mem_t* mem, int layer, uint32_t addr, uint8_t data);
...
static void _mem_map(mem_t* m, int layer, uint32_t addr, uint32_t size, const uint8_t* read_ptr, uint8_t* write_ptr) {
    CHIPS_ASSERT(m);
    ...
    for (int i = 0; i < num; i++) {
        const uint32_t offset = i * MEM_PAGE_SIZE;
        /* the page_index will wrap-around */
        const uint32_t page_index = ((addr+offset) & MEM_ADDR_MASK) >> MEM_PAGE_SHIFT;
...
void mem_map_ram(mem_t* m, int layer, uint32_t addr, uint32_t size, uint8_t* ptr) {
    CHIPS_ASSERT(ptr);
    _mem_map(m, layer, addr, size, ptr, ptr);
}

void mem_map_rom(mem_t* m, int layer, uint32_t addr, uint32_t size, const uint8_t* ptr) {
    CHIPS_ASSERT(ptr);
    _mem_map(m, layer, addr, size, ptr, 0);
}

void mem_map_rw(mem_t* m, int layer, uint32_t addr, uint32_t size, const uint8_t* read_ptr, uint8_t* write_ptr) {
...
uint8_t* mem_readptr(mem_t* m, uint32_t addr) {
    CHIPS_ASSERT(m);
    return (uint8_t*) &(m->page_table[addr>>MEM_PAGE_SHIFT].read_ptr[addr&MEM_PAGE_MASK]);
}

void mem_write_range(mem_t* m, uint32_t addr, const uint8_t* src, int num_bytes) {
